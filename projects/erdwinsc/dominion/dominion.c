...
void playAdventurer(struct gameState *state, int currentPlayer) {
    ...
    while(drawntreasure < 3){
        ...
        if (cardDrawn == copper || cardDrawn == silver || cardDrawn == province) {
          drawntreasure++;
        }
        else {
...
void playSmithy(struct gameState *state, int currentPlayer, int handPos) {
    ...
    //+3 Cards
    for (i = 0; i < 3; i++) { //changed <= to <, this was a bug that allowed players to draw an inappropriate number of cards when playing smithy. Chad Erdwins 11/17/2017
	drawCard(currentPlayer, state);
    }
...
void playFeast(struct gameState *state, int currentPlayer, int choice1) {
    int i;
    int x;
    int temphand[MAX_HAND];// moved above the if
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ){
    ...
    case feast:
      playFeast(state, currentPlayer, choice1);	
      return 0;
    ...
    case tribute:
      playTribute(state, currentPlayer);
      return 0;
